...
const ServerStaticFunctions = {
	...
	httpGET: function(url, requestData, callback){
		const tarURL = this.urlFromParams(url, requestData);
		
		const absTarURL = tarURL;
		console.log('httpGET - requesting: ', absTarURL);
		
		axios.get(absTarURL, requestData)
			  		.then((resp, err) =>{
			  			if(!!err){
			  				console.error('httpGET -> then-ERROR: ', err);
			  				if( !!callback )
			  					callback(null, err);
			  			}else{
			  				const response = {
			  			...
			  			console.error('httpGET -> axios.GET-ERROR: ', err.code);
			  			if( !!callback )
			  				callback(null, err);
			  		});
	},
	httpGET_Arduino: function(arduinoIP, baseURL, arduinoData, callback){		
		console.log('httpGet_Arduino -> arduinoIP: ', arduinoIP, '	baseURL: ', baseURL, '	arduinoData: ', arduinoData);
		
		baseURL = SF.eraseLeadingSlash( SF.erasePaddingSlash(baseURL) );
		
		const tarURL = arduinoIP + '/' + baseURL;
		
		this.httpGET(tarURL, arduinoData, callback);
